...
def append_stats(
    ...
    res = b - A @ x
    Ares = A @ res
    proj_residual_norms.append(norm(res[:-1]))
...
ax1.set_ylabel(
    "$\\Vert\\mathrm{P}_{\\mathcal{R}(A)}r_k\\Vert \\mathbin{/} \\Vert\\mathrm{P}_{\\mathcal{R}(A)}b\\Vert$"
)
...
ax1.semilogy(
    np.arange(len(proj_residual_norms_minares)),
    proj_residual_norms_minares / proj_residual_norms_minares[0],
    "-",
    markersize=3,
    label="\\textsc{MinAres}",
    zorder=-1,
...
proj_residual_norms_minres = [norm(b[:-1])]
...
def append_stats_minres(x):
    res = b - A @ x
    Ares = A @ res
    proj_residual_norms_minres.append(norm(res[:-1]))
...
ax1.semilogy(
    np.arange(len(proj_residual_norms_minres)),
    proj_residual_norms_minres / proj_residual_norms_minres[0],
    "--",
    label="\\textsc{Minres}",
    zorder=4,
)
...
proj_residual_norms_lsmr = [norm(b[:-1])]
...
def append_stats_lsmr(x):
    res = b - A @ x
    Ares = A @ res
    proj_residual_norms_lsmr.append(norm(res[:-1]))
...
ax1.semilogy(
    np.arange(len(proj_residual_norms_lsmr)),
    proj_residual_norms_lsmr / proj_residual_norms_lsmr[0],
    ":",
    label="\\textsc{Lsmr}",
    zorder=1,
)
...
proj_residual_norms_lsqr = [norm(b[:-1])]
...
def append_stats_lsqr(x):
    res = b - A @ x
    Ares = A @ res
    proj_residual_norms_lsqr.append(norm(res[:-1]))
...
ax1.semilogy(
    np.arange(len(proj_residual_norms_lsqr)),
    proj_residual_norms_lsqr / proj_residual_norms_lsqr[0],
    "-.",
    label="\\textsc{Lsqr}",
    zorder=0,
)
